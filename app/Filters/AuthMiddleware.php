...
class AuthMiddleware implements FilterInterface
{
    public function before(RequestInterface $request, $arguments = null)
    {
        ...
        if (!$session->get('id') || !$session->get('id')) {
            return redirect()->to('/login')->with('error', 'Session Expired. Please login again. Code AM1');
        }
        ...
        if ($cacheData) {
            ...
            if ($cacheData['session_id'] != $session->get('id')) {
                return redirect()->to('/login')->with('error', 'Session Expired. Please login again. Code AM2');
            }
        } else {
            // query validation
            ...
            if (!$user) {
                return redirect()->to('/login')->with('error', 'Session Expired. Please login again. Code AM3');
            }
